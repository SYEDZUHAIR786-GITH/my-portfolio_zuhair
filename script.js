...
function initNavigation() {
    ...
    const currentPath = window.location.pathname.split('/').pop() || 'portfolioindex.html';

    navLinks.forEach(link => {
        const href = link.getAttribute('href');
        
        // Highlight current page
        if (href === currentPath || (currentPath === '' && href === 'portfolioindex.html')) {
...
function initScrollAnimations() {
    ...
    const animatedElements = document.querySelectorAll(
        '.fade-in-section, .fade-in-up, .skill-card, .service-card, .project-card, .strength-item, .journey-item'
    );
...
function initInteractions() {
    ...
    const cards = document.querySelectorAll('.card, .skill-card, .service-card, .project-card');
    cards.forEach(card => {
